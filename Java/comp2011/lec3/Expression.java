...
/*
 * static boolean leftParen(char c) { return ((c == '(') || (c == '[') || (c ==
 * '{')); }
 * 
 * static boolean rightParen(char c) { return ((c == ')') || (c == ']') || (c ==
 * '}')); }
 * 
 * public static boolean isBalanced(String s) { CharStack stack = new
 * CharStack(); for (int i = 0; i < s.length(); i++) { char c = s.charAt(i);
 * if(leftParen(c)) stack.push(c); else if(rightParen(c)) { int d = c -
 * stack.pop(); if (d < 0 || d > 2) return false; } } return stack.isEmpty(); }
 */