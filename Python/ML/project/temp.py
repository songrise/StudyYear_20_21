...
def process(data, mode):
    ...
    Parameter: data: raw data; mode: 0 for training data, 1 for testing data
    """
    ...
    print(episodes_mean)
    data.loc[data["episodes"] == "Unknown", "episodes"] = 0
    # data.fillna(episodes_mean, inplace=True)
    ...
    if mode == 0:
        ...
        data = data.drop(["genre", "type", "name", "rating"], axis=1)
    ...
    # transformer = ColumnTransformer(
    #     transformers=[('cat', RobustScaler(), ["type"])])
    # type_transformed = transformer.fit_transform(data)
    # data.type = type_transformed
...
processed_train.to_csv("data/processed_teleplay.csv", index=False)
processed_test.to_csv("data/processed_newTeleplay.csv", index=False)
